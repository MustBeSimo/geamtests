  ...
  try {
    ...
    if (isDemo) {
      ...
      try {
        const llmResponse = await fetchWithTimeout(
          ...
          15000
        ); // 15 second timeout for demo

        if (!llmResponse.ok) {
          const errorData = await llmResponse
            .json()
          ...
          return NextResponse.json({
            message:
              "I'm here to support you! As your AI companion, I use CBT-inspired techniques to help you explore your thoughts and feelings. What would you like to talk about today?",
            isDemo: true,
            fallback: true,
        ...
        console.error('Demo API request failed:', {
          error: demoError,
          message:
            demoError instanceof Error ? demoError.message : 'Unknown error',
          apiKeyExists: !!process.env.TOGETHER_API_KEY,
        });
        ...
        return NextResponse.json({
          ...
          fallback: true,
          debugInfo:
            demoError instanceof Error ? demoError.message : 'Network error',
        });
      }
    }
    ...
    if (!balanceData || balanceData.balance <= 0) {
      return ErrorResponses.paymentRequired(
        'Insufficient balance. Please purchase more messages.'
      );
    }
    ...
    if (!Array.isArray(userMessages) || userMessages.length === 0) {
      return ErrorResponses.badRequest(
        'Invalid message format. Expected array of messages.'
      );
    }
    ...
    if (!llmResponse.ok) {
      ...
      return NextResponse.json({
        message:
          "I'm here to support you on your mental wellness journey. Sometimes I might take a moment to respond, but I'm always ready to listen and help you explore your thoughts and feelings. What's on your mind today?",
    ...
    if (
      ...
      return NextResponse.json({
        message:
          "I'm experiencing some technical difficulties right now, but I'm here to support you. Let me know what's on your mind, and I'll do my best to help you explore your thoughts and feelings.",
        fallback: true,
      });
    }
