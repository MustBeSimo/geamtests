...
const WeatherHoroscopeCard = React.lazy(
  () => import('@/components/WeatherHoroscopeCard')
);
...
import {
  GeneralDisclaimer,
  CrisisBanner,
} from '@/components/ComplianceDisclaimer';
...
export default function HomeClient() {
  ...
  const handleBuyPlus = async () => {
    if (!user) {
      alert(
        'Please sign in first to purchase the Plus plan. Click the "Sign In" button at the top right.'
      );
      return;
    }
    setBuyingPlus(true);
  ...
  const handleStartDemo = (goal?: string) => {
    if (typeof window !== 'undefined') {
      ...
      if (parsedData.count >= 3) {
        alert(
          "You've used your 3 free demo messages! Sign in to get 20 free messages and continue your wellness journey."
        );
        signInWithGoogle();
        return;
      }
      ...
      <div className="relative z-10">
        <div className="container mx-auto px-4 pb-8 max-w-7xl">
          ...
          {isChatVisible && (
            <section className="mb-12 lg:mb-16">
              <div
                className={`bg-white/80 dark:bg-gray-800/80 rounded-2xl p-8 shadow-lg border-2 ${getAvatarColorScheme(selectedAvatar).borderColor}`}
              >
                <div className="flex items-center justify-center gap-3 mb-8">
                  <div
                    className={`w-12 h-12 rounded-full bg-gradient-to-br ${getAvatarColorScheme(selectedAvatar).gradient} p-1`}
                  >
                    <div className="w-full h-full rounded-full flex items-center justify-center overflow-hidden">
                      <Image
                        src={
                          currentAvatar?.src ||
                          '/images/avatars/gigi-avatar-logo.png'
                        }
                  ...
                  <h2
                    className={`text-2xl font-bold ${getAvatarColorScheme(selectedAvatar).textColor}`}
                  >
            ...
            <div className="bg-white/80 dark:bg-gray-800/80 rounded-2xl p-8 shadow-lg">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 max-w-6xl mx-auto">
                ...
                <div className="bg-gradient-to-br from-emerald-50 to-teal-50 dark:from-emerald-900/20 dark:to-teal-900/20 rounded-xl p-6 border-2 border-emerald-200 dark:border-emerald-600 relative h-fit">
                  ...
                  <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
                    200 messages + 60 mood check-ins and reports
                  </p>
                  <ul className="space-y-2 text-sm text-gray-600 dark:text-gray-400 mb-4">
                    <li className="flex items-center gap-2">
                      ...
                      <span>Advanced CBT-based guidance</span>
                    </li>
                    <li className="flex items-center gap-2">
                      <svg
                        className="w-4 h-4 text-green-500"
                  ...
                  <div className="mt-4">
                    <button
                      ...
                      disabled={buyingPlus}
                      className={`w-full py-3 px-4 rounded-lg font-semibold text-white transition-all duration-200 ${buyingPlus ? 'bg-gray-400 cursor-not-allowed' : 'bg-emerald-600 hover:bg-emerald-700 active:scale-95 shadow-lg hover:shadow-xl'}`}
                    >
                ...
                <div className="bg-gray-50 dark:bg-gray-700 rounded-xl p-6 border-2 border-gray-200 dark:border-gray-600 h-fit">
                  ...
                  <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
                    500 messages + 150 mood check-ins and reports
                  </p>
                  <ul className="space-y-2 text-sm text-gray-600 dark:text-gray-400 mb-4">
                    <li className="flex items-center gap-2">
                      <svg
